  ...
  return (
    <div className="page-wrap !px-0 !pb-0 !max-w-none">
      ...
      {attachments.length > 0 ? (
        /**
         * Revisi: TIDAK ada pratinjau untuk semua jenis lampiran materi
         * (pdf, gambar, video, tautan, file) — cukup daftar nama file yang
         * bisa dibuka/diunduh. Nama tampilan file dapat dikustom oleh guru/admin.
         */
        <div className="mt-3 max-w-[760px] space-y-2.5">
          <p className="text-[13px] font-semibold">Lampiran materi ({attachments.length})</p>
          {attachments.map((file, index) => {
            const kind = kindOf(file);
            const tone = KIND_TONE[kind];
            const isTautan = kind === "tautan";
            return (
              <div key={`${file.url}-${index}`} className="card card-pad flex items-center gap-3 !py-3">
